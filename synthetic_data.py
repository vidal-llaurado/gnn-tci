...
class SyntheticDataTest:
    def __init__(self, n, m):
        """
        Initialize SyntheticDataTest with a Barabasi-Albert graph.
        
        This class generates test data WITHOUT repayment edges, simulating
        the prediction scenario where only goods_service edges are available
        and the GNN must predict which repayment edges should exist.
        ...
        self.m = m
        self.G = self._ba_graph(n, m)
        self.G = self._goods_service_edges_only(self.G)
        self.G = self._node_features(self.G)
        self.G = self._edge_features(self.G)
    ...
    def _goods_service_edges_only(self, G):
        """
        Create goods/service edges only (no repayment edges).
        This simulates the prediction scenario where repayment edges are unknown.
        Assign transaction IDs to keep track of edges.
        
        Args:
            G: NetworkX DiGraph
            
        Returns:
            NetworkX DiGraph: Graph with only goods_service edges and transaction IDs
        """
        # Create a new directed graph
        G_new = nx.DiGraph()
        ...
        # Create only goods_service edges (no repayment edges)
        for u, v in original_edges:
            # Only add goods/service flow from u to v
            # No repayment edges - these will be predicted by the GNN
            G_new.add_edge(u, v, transaction_id=transaction_id, 
                          edge_type='goods_service')
    ...
    def _edge_features(self, G):
        """
        Generate random features for goods_service edges only.
        Features follow expected distributions for transaction data.
        ...
        Returns:
            NetworkX DiGraph: Graph with edge features
        """
        # Process goods_service edges only (no repayment edges in test data)
        for u, v, data in G.edges(data=True):
            edge_type = data.get('edge_type', 'unknown')
    ...
    def visualize(self, figsize=(12, 8), node_size=500, font_size=8):
        """
        Visualize the network (only goods_service edges).
        ...
        # Draw edges (only goods_service edges in test data)
        goods_edges = [(u, v) for u, v, d in self.G.edges(data=True) 
                      if d.get('edge_type') == 'goods_service']
        ...
        plt.title('Financial Transaction Network (Test Data)\n(Blue: Goods/Service Flow - Repayment edges to be predicted)')
        plt.legend()
        plt.axis('off')
        plt.tight_layout()
        plt.show()
    ...
    def get_summary(self):
        ...
        # Count transactions (goods_service edges)
        transaction_ids = set()
        for u, v, d in self.G.edges(data=True):
            transaction_ids.add(d.get('transaction_id'))
        ...
        summary = {
            ...
            'num_edges': num_edges,
            'num_transactions': num_transactions,
            'num_goods_service_edges': num_edges,  # All edges are goods_service in test data
            'num_repayment_edges': 0,  # No repayment edges in test data
            'avg_credit_rating': np.mean(credit_ratings) if credit_ratings else 0,
            'avg_transaction_amount': np.mean(transaction_amounts) if transaction_amounts else 0,
        }
    ...
    def save(self, filename='test', data_dir='data'):
        """
        Save the generated network to the data folder.
        
        Args:
            filename: Base filename (without extension). Default is 'test'
            data_dir: Directory to save the file. Default is 'data'
        """
        # Create data directory if it doesn't exist
        data_path = Path(data_dir)
